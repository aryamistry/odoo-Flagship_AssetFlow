...
const databaseUrl =
  ...
  "postgresql://assetflow:assetflow@localhost:5432/assetflow?schema=public";
const prisma = new PrismaClient({
  adapter: new PrismaPg({ connectionString: databaseUrl }),
});
const pepper =
  process.env.SESSION_PEPPER ?? "development-only-change-me-please-32-bytes";
...
async function main() {
  ...
  const days = (amount: number) =>
    new Date(now.getTime() + amount * 86_400_000);

  await prisma.organization.upsert({
    where: { id: ids.org },
    update: { name: "AssetFlow Demo" },
    create: { id: ids.org, name: "AssetFlow Demo", slug: "assetflow-demo" },
  });
  for (const department of [
    ...
    await prisma.department.upsert({
      ...
      update: department,
      create: { ...department, organizationId: ids.org },
    });
  ...
  for (const location of [
    ...
    await prisma.location.upsert({
      ...
      update: location,
      create: { ...location, organizationId: ids.org },
    });
  ...
  for (const user of [
    {
      ...
      primaryDepartmentId: ids.facilities,
    },
    {
      id: ids.manager,
      firstName: "Maya",
    ...
    },
    {
      ...
      primaryDepartmentId: ids.engineering,
    },
    {
      id: ids.employee,
      firstName: "Riya",
    ...
    await prisma.user.upsert({
      ...
      create: { ...user, passwordHash, organizationId: ids.org },
    });

  await prisma.userRole.deleteMany({ where: { organizationId: ids.org } });
  await prisma.userRole.createMany({
    data: [
      { organizationId: ids.org, userId: ids.admin, role: "EMPLOYEE" },
      { organizationId: ids.org, userId: ids.admin, role: "ADMIN" },
      { organizationId: ids.org, userId: ids.manager, role: "EMPLOYEE" },
      { organizationId: ids.org, userId: ids.manager, role: "ASSET_MANAGER" },
      { organizationId: ids.org, userId: ids.head, role: "EMPLOYEE" },
      {
  ...
  for (const category of [
    ...
    await prisma.assetCategory.upsert({
      ...
      update: category,
      create: { ...category, organizationId: ids.org },
    });

  const assets = [
    {
      id: ids.laptopAvailable,
      assetTag: "AF-0001",
      ...
      isBookable: false,
      currentLocationId: ids.store,
      owningDepartmentId: ids.engineering,
    },
    {
      id: ids.laptopAllocated,
      assetTag: "AF-0002",
      name: "Design Laptop",
      categoryId: ids.electronics,
    ...
    },
    {
      ...
      isBookable: true,
      currentLocationId: ids.store,
      owningDepartmentId: ids.facilities,
    },
    {
      id: ids.meetingRoom,
      assetTag: "AF-0004",
      name: "Room B2",
      categoryId: ids.roomCategory,
      status: "AVAILABLE" as const,
      condition: "GOOD" as const,
      isBookable: true,
      currentLocationId: ids.room,
      owningDepartmentId: ids.facilities,
    },
    {
      id: ids.vehicleAsset,
  ...
  for (const [assetId, policy] of [
    ...
    await prisma.resourceProfile.upsert({
      where: { assetId },
      update: { ...policy, availabilityJson: weekdayHours },
      create: {
        assetId,
        organizationId: ids.org,
        timezone: "Asia/Kolkata",
        ...policy,
        availabilityJson: weekdayHours,
      },
    });
  ...
  await prisma.allocation.upsert({
    ...
    update: { expectedReturnAt: days(-2) },
    create: {
      id: allocationId,
      organizationId: ids.org,
      assetId: ids.laptopAllocated,
      allocatedToUserId: ids.employee,
      allocatedByUserId: ids.manager,
  ...
  await prisma.transferRequest.upsert({
    ...
    create: {
      ...
      requestedByUserId: ids.employee,
      toDepartmentId: ids.finance,
  ...
  await prisma.booking.upsert({
    ...
    update: {
      startAt: hours(24),
      endAt: hours(26),
      effectiveStartAt: hours(24),
      effectiveEndAt: hours(26),
    },
    create: {
      id: "00000000-0000-4000-8000-000000000701",
      organizationId: ids.org,
      assetId: ids.meetingRoom,
      requestedByUserId: ids.employee,
      bookedForUserId: ids.employee,
  ...
  await prisma.maintenanceRequest.upsert({
    ...
    create: {
      ...
      assetId: ids.printer,
      requestedByUserId: ids.employee,
      issueDescription: "Paper feed mechanism jams",
      priority: "HIGH",
      status: "TECHNICIAN_ASSIGNED",
      assignedTechnicianUserId: ids.manager,
  ...
  await prisma.maintenanceEvent.deleteMany({
    where: { maintenanceRequestId: "00000000-0000-4000-8000-000000000801" },
  });
  await prisma.maintenanceEvent.create({
    data: {
      organizationId: ids.org,
      maintenanceRequestId: "00000000-0000-4000-8000-000000000801",
      toStatus: "TECHNICIAN_ASSIGNED",
      performedByUserId: ids.manager,
    },
  });
  await prisma.maintenanceRequest.upsert({
    ...
    create: {
      ...
      requestedByUserId: ids.employee,
      issueDescription: "Image flickers after warm-up",
      priority: "MEDIUM",
    },
  });
  ...
  await prisma.auditItem.upsert({
    ...
    create: {
      ...
      auditCycleId: auditId,
      assetId: ids.laptopAllocated,
      snapshotAssetTag: "AF-0002",
      snapshotAssetName: "Design Laptop",
      expectedStatus: "ALLOCATED",
      expectedCondition: "GOOD",
      expectedLocationId: ids.store,
      expectedDepartmentId: ids.engineering,
      expectedHolderUserId: ids.employee,
      result: "MISSING",
      checkedByUserId: ids.employee,
      checkedAt: now,
    },
  });
  await prisma.auditDiscrepancy.upsert({
    ...
    create: {
      ...
      auditItemId: itemId,
      type: "MISSING",
      details: "Asset not found at expected location",
    },
  });

  process.stdout.write(
    `Seeded AssetFlow Demo. Login with admin@assetflow.local / ${password}\n`,
  );
}
